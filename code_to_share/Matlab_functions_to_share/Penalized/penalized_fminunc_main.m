...
step_size = 1e-8*ones(length(guess),1);
options = optimoptions('fminunc', ...
        ...
        'StepTolerance',1e-6, ...
        'OptimalityTolerance', 1e-6, ...  % convergence tolerance on first order optimality'ConstraintTolerance', 1e-16, ...  % convergence tolerance on constraints
        'FiniteDifferenceType', 'forward', ...  % if finite differencing, can also use central
        'FiniteDifferenceStepSize', step_size,...
        'Diagnostics', 'on'); % display diagnotic information
global funcall;
funcall = 0;
[gen_opt, cost_opt, ~, output] = fminunc(@(gen)obj_penalized(gen,time,net_load),guess,options);
